...
class TestKdiff:
    def test_basics(self, tmpdir, mocker):
        with tmpdir.as_cwd():
            with mock.patch(
                    'src.helper.kdiff.KdiffHelper.path',
                    new=mocker.PropertyMock(return_value=Path(str(tmpdir.mkdir('kdiff').join('kdiff3.exe'))))):
                kdiff = KdiffHelper()
    ...
    def test_install(self, mocker: MockFixture, tmpdir):
        with tmpdir.as_cwd():
            ...
            with mock.patch(
                ...
                # return
                assert kdiff.path.abspath() == Path(str(tmpdir.join('kdiff').join('kdiff3.exe'))).abspath()
