...
class Repo(IRepo):
    def __init__(self, user: str):
        ...
        if user == GHSession().user:
            ...
            self._remote = GHSession().own_meta_repo
        else:
            try:
                self._remote = GHSession().get_repo('EASIMETA', user=user)
            except FileNotFoundError:
                logger.error('user {} has not EASIMETA repository'.format(user))
                raise

        logger.debug('creating local GitRepository object')
        self._local_git_repo = GitRepository(self.local_git_repo_path, auto_init=False)

        if not self.local.is_init:
            logger.debug('no local repo, cloning remote')
            self._clone_from_github(user)
        else:
        ...
        def gh_user_changed(sender, *args, **kwargs):
            try:
                ...
                self._remote = gh_session.get_repo('EASIMETA', user=self._user)
            except FileNotFoundError:
                logger.error('user {} has not EASIMETA repository'.format(self._user))
